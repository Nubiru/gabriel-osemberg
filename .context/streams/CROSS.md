...
| SHOWCASE | DATA | ProjectMetric model (LOC, coverage, test_count, src_file_count) | L1 | DELIVERED | L1 delivered: get_project_metrics(), get_project_tags(), get_projects_by_tag(), get_aggregated_stats(). 25 tags, 10 metrics seeded. |
...
| INFRA | DATA | Seed data idempotency (INSERT OR IGNORE) | L0 | RESOLVED | SQLx migration tracking is inherently idempotent. New migrations (004+) use INSERT OR IGNORE. Response in INFRA inbox. |
